...
export class AvatarManager {
  ...
  async initAvatar() {
    ...
    try {
      ...
      setTimeout(() => this.uiManager?.hideLoadingScreen(), 500);
    }
  }
  ...
  async initializeOtherManagers() {
    ...
    this.webSocketManager?.setCallbacks({
      ...
      onClaudeResponse: (response, avatarEmoji, usedKnowledgeBase, catalogueData, usedEvents) => {
        this.chatManager?.handleClaudeResponse(response, avatarEmoji, usedKnowledgeBase, catalogueData, usedEvents);
        if (usedKnowledgeBase) {
          console.log('🧠 Response included knowledge base information');
        }
        if (catalogueData) {
          console.log('📚 Response included catalogue data');
        }
        if (usedEvents) {
      ...
      onCatalogueThinking: (show, message) => {
        if (show) {
          this.uiManager?.showCatalogueThinking(message);
        } else {
          // Check if this is a delayed hide request
          if (message === 'delayed') {
            this.uiManager?.hideCatalogueThinking(3000); // 3 second delay
          } else {
            this.uiManager?.hideCatalogueThinkingImmediate();
          }
        }
      },
