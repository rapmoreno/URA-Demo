export class SpeechBubbleManager {
  ...
  showSpeechBubble(text, delay = 0) {
    ...
    if (delay > 0) {
      this.speechBubbleTimeout = setTimeout(showBubbleContent, delay);
    } else {
      showBubbleContent();
    }
  }
  ...
  showChunkedSpeechBubble(fullText, wordTimes, words) {
    ...
    if (this.currentChunks.length === 0) {
      this.showSpeechBubble(fullText, 1200);
      return;
    }
    
    // Show first chunk with a 1200ms delay to sync with voice
    this.showSpeechBubble(this.currentChunks[0], 1200);
    ...
    if (this.currentChunks.length > 1) {
      ...
      for (let i = 1; i < this.currentChunks.length; i++) {
        ...
        // Add the initial delay to synchronize with the first chunk
        timing += 800;
        
        // Ensure minimum duration between chunks (2.5 seconds)
        const minTiming = i * 2500 + 800;
