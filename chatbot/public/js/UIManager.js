export class UIManager {
  constructor() {
    ...
    this.catalogueThinkingTimeout = null;
  ...
  // =====================================================
  // CATALOGUE THINKING BUBBLE
  // =====================================================
  showCatalogueThinking(message = 'let me check our books') {
    const catalogueThinking = document.getElementById('catalogueThinking');
    const catalogueText = catalogueThinking?.querySelector('.catalogue-thinking-text');
    
    if (catalogueText) {
      catalogueText.textContent = message;
    }
    
    if (catalogueThinking) {
      catalogueThinking.classList.add('visible');
    }
    
    // Update status indicator to catalogue mode
    this.updateClaudeStatus('Searching books...', 'knowledge-base');
    
    console.log('📚 Catalogue thinking bubble shown:', message);
  }
  
  hideCatalogueThinking(delay = 3000) {
    // Clear any existing timeout
    if (this.catalogueThinkingTimeout) {
      clearTimeout(this.catalogueThinkingTimeout);
    }
    
    // Set a timeout to hide the thinking bubble after the specified delay
    this.catalogueThinkingTimeout = setTimeout(() => {
      const catalogueThinking = document.getElementById('catalogueThinking');
      
      if (catalogueThinking) {
        catalogueThinking.classList.remove('visible');
      }
      
      console.log('📚 Catalogue thinking bubble hidden after delay');
      this.catalogueThinkingTimeout = null;
    }, delay);
  }
  
  hideCatalogueThinkingImmediate() {
    // Clear any existing timeout
    if (this.catalogueThinkingTimeout) {
      clearTimeout(this.catalogueThinkingTimeout);
      this.catalogueThinkingTimeout = null;
    ...
    console.log('📚 Catalogue thinking bubble hidden immediately');
  }
  
  updateCatalogueThinkingText(text) {
    const catalogueText = document.querySelector('#catalogueThinking .catalogue-thinking-text');
    if (catalogueText) {
      catalogueText.textContent = text;
      console.log('📚 Catalogue thinking text updated:', text);
    }
  }
