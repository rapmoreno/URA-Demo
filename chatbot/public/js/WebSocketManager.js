export class WebSocketManager {
  ...
  setCallbacks({ onStatusUpdate, onMessage, onClaudeResponse, onLoading, onKnowledgeBaseThinking, onCatalogueThinking, onEventsThinking }) {
    this.onStatusUpdate = onStatusUpdate;
    this.onMessage = onMessage;
    this.onClaudeResponse = onClaudeResponse;
    this.onLoading = onLoading;
    this.onKnowledgeBaseThinking = onKnowledgeBaseThinking;
    this.onCatalogueThinking = onCatalogueThinking;
    ...
    console.log('🔌 WebSocket callbacks set:', {
      ...
      onCatalogueThinking: !!onCatalogueThinking,
  ...
  handleWebSocketMessage(data) {
    switch (data.type) {
      ...
      case 'catalogue_thinking':
        if (data.status) {
          // Show catalogue thinking bubble
          if (this.onCatalogueThinking) {
            this.onCatalogueThinking(true, data.message);
          }
          
          // Trigger Checking 💡 animation simultaneously
          if (window.triggerExpression) {
            console.log('💡 Triggering Checking animation for catalogue search');
            window.triggerExpression('💡');
          }
        } else {
          // Hide catalogue thinking bubble
          if (this.onCatalogueThinking) {
            this.onCatalogueThinking(false);
          }
        }
        break;
      ...
      case 'response':
        ...
        // Only hide catalogue thinking with delay if it was actually used
        if (this.onCatalogueThinking && data.usedCatalogue) {
          this.onCatalogueThinking(false, 'delayed');
        } else if (this.onCatalogueThinking) {
          // If no catalogue was used, hide immediately
          this.onCatalogueThinking(false, 'immediate');
        }
        ...
        if (this.onClaudeResponse) {
          this.onClaudeResponse(data.response, data.avatarEmoji, data.usedKnowledgeBase, data.catalogueData, data.usedEvents);
        }
      ...
      case 'error':
        ...
        // Hide catalogue thinking if shown
        if (this.onCatalogueThinking) {
          this.onCatalogueThinking(false);
        }
  ...
  async sendChatHTTP(message, conversationId) {
    try {
      ...
      const catalogueKeywords = ['book', 'novel', 'author', 'read', 'magazine', 'dvd', 'cd', 'ebook', 'catalogue', 'search', 'find', 'material', 'item', 'collection', 'title', 'story', 'fiction', 'harry', 'potter', 'programming', 'javascript', 'python', 'cooking', 'romance', 'mystery', 'thriller', 'biography', 'history', 'science'];
      ...
      // Show catalogue thinking bubble if catalogue keywords detected
      if (containsCatalogueKeywords && this.onCatalogueThinking) {
        console.log('📚 HTTP: Detected catalogue keywords in message:', message);
        console.log('📚 HTTP: Showing catalogue thinking bubble');
        this.onCatalogueThinking(true, 'let me check our books');
        
        // Trigger Checking 💡 animation simultaneously
        if (window.triggerExpression) {
          console.log('💡 HTTP: Triggering Checking animation for catalogue search');
          window.triggerExpression('💡');
        }
      }
      ...
      // Hide catalogue thinking bubble after response (with delay if catalogue was used)
      if (containsCatalogueKeywords && this.onCatalogueThinking) {
        // Check if catalogue was actually used (either from API flag or response content)
        const catalogueWasUsed = data.usedCatalogue || 
          (data.response && (
            data.response.toLowerCase().includes('found') && 
            (data.response.toLowerCase().includes('book') || data.response.toLowerCase().includes('result'))
          ));
          
        if (catalogueWasUsed) {
          console.log('📚 HTTP: Catalogue was used, hiding with delay');
          this.onCatalogueThinking(false, 'delayed');
        } else {
          console.log('📚 HTTP: Catalogue not used, hiding immediately');
          this.onCatalogueThinking(false, 'immediate');
        }
      }
