...
async function initializeApp() {
  ...
  // ================================
  // BROWSER MEMORY SYSTEM - DISABLED FOR DEMO
  // ================================
  // Memory system disabled to ensure fresh conversations for demo
  window.BrowserMemory = null; // Explicitly disable
  console.log('🗑️ BrowserMemory DISABLED - Fresh conversations for demo');
  ...
  // Clear any existing memory and test auto-greeting
  try {
    localStorage.removeItem('hr-chatbot-memory');
    console.log('🗑️ Cleared any existing conversation memory');
  } catch (error) {
    console.log('ℹ️ No existing memory to clear');
  }
  
  // Auto-greeting disabled - manual interaction only for demo
  console.log('👋 Auto-greeting disabled - waiting for manual user interaction');
}
...
window.toggleVoice = async () => {
  console.log('🎤 Global toggleVoice called');
  if (speechRecognitionManager) {
    await speechRecognitionManager.toggleVoice();
  } else {
    console.error('🎤 speechRecognitionManager not found');
  }
};
...
// Listen for audio activation message from parent window
window.addEventListener('message', (event) => {
  if (event.data && event.data.action === 'enableAudio') {
    console.log('🔊 Audio activation received from parent window');
    
    // Create and resume audio context on user gesture
    if (window.ttsManager) {
      window.ttsManager.preWarmAudioContext();
    }
    
    // Try to enable audio context directly
